...
import com.example.faceauthentication.CameraActivity;
...
//    private MaskDetectionModel maskDetectionModel;
    ...
    private ArrayList<android.util.Pair<String, float[]>> faceList;
...
//    private final boolean isMaskDetectionOn = true;
    ...
    FaceComparationCallback faceComparationCallback;
    public Comparison(Context context, Bitmap bitmap, ArrayList<Pair<String, float[]>> faceList1, FaceComparationCallback faceComparationCallback, FaceNetModel model) throws IOException {
        this.context = context;
        this.faceComparationCallback = faceComparationCallback;
        this.model = model;
//        maskDetectionModel = new MaskDetectionModel(context);
    ...
    public void runModel(List<Face> faces, Bitmap cameraFrameBitmap) {
        ...
        if(faces.isEmpty())
        {
            ...
            faceComparationCallback.onComparationFailed("no faces detected");
...
//                    } while (speakingEnd);
                    faceComparationCallback.onComparationCompleted(bestScoreUserName);
                }
            } catch (Exception e) {
                Log.d("Model", "Exception in FrameAnalyser : " + e.getMessage());
                //textToSpeech.speak("Unknown", TextToSpeech.QUEUE_FLUSH, null);
                faceComparationCallback.onComparationFailed("Unknown");
            }
        }
